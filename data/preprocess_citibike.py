...
def process_data_bike(root="", year=2019, event_num=500, dl = False, data_path = 'data/citibike.npz'):
    np.random.seed(0)
    if dl:
        download()
        ...
        for range_ in range(5000):
            ...
            for i in range(20):
                ...
                sequences[seq_name + f"_{i:03d}"] = add_spatial_noise(seq[subsample_idx],
                                                                      std=[0., std_x * 0.02, std_y * 0.02, 0., 0.])
...
if __name__ == "__main__":
    mean, std = process_data_bike(event_num=500, dl = False, data_path = 'citibike.npz')
